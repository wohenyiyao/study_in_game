"""种子数据：管理员账号 + Python 闯关内容（2 章 4 关 20 题）。"""
from .database import Base, engine, SessionLocal, ensure_database
from .models import User, Chapter, Level, Question
from .auth import hash_password

# 章节/关卡/题目：chapter -> [(level_title, description, pass_ratio, [ (content, options, answer, explanation) ])]
SEED = [
    {
        "title": "Python 基础语法",
...
def seed() -> None:
    ...
    try:
        ...
        if db.query(Chapter).first() is not None:
            print("已存在内容，跳过种子题目（如需重置请删除数据库文件后重跑）")
            return
        for ch_i, ch_data in enumerate(SEED):
            ch = Chapter(title=ch_data["title"],
                         description=ch_data["description"], order=ch_i)
            db.add(ch)
            db.flush()
            for lv_i, lv_data in enumerate(ch_data["levels"]):
                lv = Level(chapter_id=ch.id, title=lv_data["title"],
                           description=lv_data["description"],
                           order=lv_i, pass_ratio=lv_data["pass_ratio"])
                db.add(lv)
                db.flush()
        ...
        db.commit()
        print("种子数据完成：2 章 4 关 20 题 + 管理员(admin/admin123)")
    finally:
        db.close()
